...
func FetchRecommendationsFromElastic(userId string) ([]string, error) {
	...
	if len(recommendations) == 0 {
		_, fallbackRecommendations, err2 := getFallbackRecommendationsFromUserIndex(userId, esClient, recommendations)
		if err2 != nil {
			return fallbackRecommendations, err2
		}
	}
...
func getFallbackRecommendationsFromUserIndex(userId string, esClient *elasticsearch.Client, recommendations []string) ([]string, []string, error) {
	...
	if fallbackRes.IsError() {
		return nil, nil, fmt.Errorf("error fetching fallback recommendations: %s", fallbackRes.String())
	}
